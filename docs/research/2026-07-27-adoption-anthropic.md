...
**Befund über unsere eigenen Dateien:** **KORREKTUR 2026-08-02 (Messung schlaegt Dokumentation):** der Satz war NICHT wahr. Gemessen in einer echten headless Sitzung (`agent: backend-developer`, `--tools ""`, Kontrollwort in der Agent-Datei, Gegenkontrollwort am SKILL-Ende): die Agent-Datei kam woertlich an, das SKILL nicht. Der `skills:`-Frontmatter-Weg liefert einer Sitzung, an die eine Rolle gebunden ist, nichts. Der SUBAGENT-SPAWN-Weg -- den der zitierte Doku-Satz beschreibt -- bleibt ungemessen; siehe `tools/provider_observations.json`. Die 23 SKILLs tragen den Satz seit 2026-08-02 nicht mehr. Urspruenglicher Befund der Runde: alle neun Descriptions enden auf „Preloaded into the X subagent." Das ist wahr und als Trigger wertlos — für preloaded Subagent-Skills triggert die Description ohnehin nicht (Doku: „Subagents with preloaded skills work differently: the full skill content is injected at startup"). Der Nutzen ist bei uns also **kleiner als es aussieht** und beschränkt sich auf `project-manager` (Session-Agent) und alles, was per `/name` aufgerufen wird. Kostet null Körperbudget (Frontmatter, 1 024 Zeichen erlaubt) — dort könnte statt der Preload-Notiz die Abgrenzung „wann NICHT diese Rolle" stehen.
