...
- **`description` als Trigger-Fläche aufpolieren** — größtenteils wirkungslos: bei preloaded Subagent-Skills wird der volle Inhalt beim Start injiziert, die Description triggert dort gar nicht. Relevant bleibt sie nur für `project-manager` (Session-Agent) und `/name`-Aufrufe; dort könnte statt „Preloaded into the X subagent." die Abgrenzung „wann NICHT diese Rolle" stehen — Frontmatter, null Körperbudget.
